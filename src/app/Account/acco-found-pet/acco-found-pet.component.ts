...
@Component({
  selector: 'app-acco-found-pet',
  templateUrl: './acco-found-pet.component.html',
  styleUrls: ['./acco-found-pet.component.css','./style.css']
})
export class AccoFoundPetComponent implements OnInit {
...
// this.authService.animauxlostPost(this.userfound, bred, size, species, age, descri, couleur, gender, distmarkings, formData, datefound, addrs)
const {bred, size, species, age, descri, couleur, gender, distmarkings, datefound, addrs } = this.form;
this.idu = this.tokenStorage.getUser().user.id
this.userfound = this.idu

const formData = new FormData();
formData.append('image', this.selectedImagepet);
formData.append('userfound', this.idu);
...
formData.append('addrs', addrs);
  ...
  this.http.post<any>('https://adoption.pythonanywhere.com/api/AnimauxfoundList/', formData).subscribe(
  data => {
    console.log(data);
  ...
  formData1.append('daten', datefound);
